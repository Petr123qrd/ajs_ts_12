...
test("На товары с isQuantiable = true и с одинаковыми id, не должно быть ограничения по количеству", () => {
  const cart = new Cart();
  const phone1 = new Gadget(4242, "phone", 200000, "iPhone 14", true);
  const phone2 = new Gadget(4242, "phone", 200000, "iPhone 14", true);
  const phone3 = new Gadget(4242, "phone", 200000, "iPhone 14", true);
  cart.add(phone1);
  cart.add(phone2);
  cart.add(phone3);
  expect(cart.items.length).toBe(3);
